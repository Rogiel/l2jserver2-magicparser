 ...
 */
package com.magicparser.util.jnetpcap;
...
/**
 * A special buffer used for reassembling the original IP datagram. The
 * reassembled buffer contains an IP header upfront and it suitable for peering
 * with a packet and then decoding.
 * 
 * <pre>
 ...
 * </pre>
 * 
 * The header comes from the first segment that was seen part of this IP
 * datagram. The original header is used as a template and then several fields
 * are reset to reflect the state of reassembled packet. The fields modified
 * are:
 * <ul>
 * <li>Ip4.flags</li>
 * <li>Ip4.length</li>
 ...
 */
public class IpReassemblyBuffer extends JBuffer implements
		Comparable<IpReassemblyBuffer> {
	/**
	 * Our custom interface that allows listeners to get our special reassembled
	 * IP packets and also provide them with the actual reassembled buffer.
	 * 
	 * @author Mark Bednarczyk
	 * @author Sly Technologies, Inc.
	 */
	public static interface IpReassemblyHandler {
		/**
		 * @param packet
		 *            the packet
		 */
	 ...
	 * nicely allows us to construct a new custom packet. Our new packets don't
	 * care about the lower OSI layers since that information is irrelavent for
	 * Ip reassembly and for the user as well. If we receive a packet that is
	 * not fragmented we simply pass it through, no sense in doing anything
	 * special with it.
	 * </p>
	 * 
	 * @author Mark Bednarczyk
	 * @author Sly Technologies, Inc.
	 */
	public static class IpReassembly implements PcapPacketHandler<Object> {
		/**
		 ...
		 */
		private IpReassemblyHandler handler;
		...
		/**
		 * Amount of time in milli seconds, after which reassembly buffers are
		 * timedout out of the queue.
		 */
		private final long timeout;

		/**
		 * Timeout queue to which all new reassembly buffers are added. The
		 * queue is prioritized according to timeout timestamp of each buffer.
		 */
		private final Queue<IpReassemblyBuffer> timeoutQueue = new PriorityQueue<IpReassemblyBuffer>();

		/**
		 * Creates a reassembly handler that reassembles all incoming IP packet.
		 * 
		 * @param timeout
		 ...
		 */
		public IpReassembly(long timeout, IpReassemblyHandler handler) {
		...
		private IpReassemblyBuffer bufferFragment(PcapPacket packet, Ip4 ip) {
			...
			/*
			 * Lets keep in mind that ip.getOffset() is a header offset into the
			 * packet buffer, while ip.offset() is the Ip4.offset field which is
			 * the fragment offset into the overall datagram, in multiples of 8
			 * bytes
			 */
		...
		/**
		 * Process an Ip4 fragment. Fragment is copied into appropriate
		 * reassembly buffer. This is also a special fragment as its the last
		 * fragment of the fragmented IP datagram.
		 * 
		 * @param packet
		 *            Ip4 fragment packet
		 * @param ip
		...
		private void dispatch(IpReassemblyBuffer buffer) {
			final JMemoryPacket packet = new JMemoryPacket(Type.POINTER);
			packet.setWirelen(buffer.size());
			packet.peer(buffer);
			packet.scan(Ip4.ID);

			if (!buffer.isComplete()) {
				System.err.println("Packet is not complete!");
				return;
			}

			final PcapHeader header = new PcapHeader(Type.POINTER);
			header.peer(buffer, 0);

			handler.nextPacket(packet, header, buffer);
		}

		/**
		 * Retrieves a reassembly buffer for this particular Ip packet. The
		 * supplied Ip4 header is used to determine if a buffer already exists
		 * and if not a new one is created.
		 * 
		 * @param ip
		 *            Ip4 header of current Ip4 fragment
		 * @return a reassembly buffer used for reassembly of this Ip4 datagram
		 */
		private IpReassemblyBuffer getBuffer(Ip4 ip) {
			...
			if (buffer == null) { // First time we're seeing this id
				 ...
				 * fragments
				 */
				final long bufTimeout = System.currentTimeMillis()
						+ this.timeout;
		...
		public void nextPacket(PcapPacket packet, Object user) {
			if (packet.hasHeader(ip)) {
				...
				if ((ip.flags() & Ip4.FLAG_MORE_FRAGMENTS) != 0) {
					...
					/*
					 * Here we have a non-fragmented IP packet so we just pass
					 * it on
					 */
				}
		...
		/**
		 * Check the timeout queue and timeout any buffers that are timedout.
		 * The timed out buffer are dispatched, incomplete to the user's
		 * handler. Buffers that are still on the queue, are incomplete but have
		 * not timedout yet are ignored.
		 */
	...
	/**
	 * Total length of the reassembled IP fragments including the ip header
	 */
	...
	/**
	 * Offset where the Ip payload begins in the reassembled IP datagram. Always
	 * constant since our IP header is constant as well.
	 */
	...
	/**
	 * Creates a new buffer for IP fragment reassebly. The buffer appends an Ip4
	 * header to the front of the buffer. The supplied ip header is only used as
	 * a template and a copy is made. This allows the buffer to retain all the
	 * vital Ip4 information found in the original Ip4 datagram before it was
	 * fragmented.
	 * 
	 * @param ip
	 *            ip header of one of the fragments to be used as a template for
	 *            the reassembled packet
	 * @param size
	 *            amount of memory to allocate for reassembly
	 * @param timeout
	 *            timestamp in millis when this buffer should be timed out
	 * @param hash
	 *            special Ip4 based hash used for identifying this buffer
	 *            quickly
	 */
	...
	/**
	 * Deep copy the supplied Ip4 header to the front of the buffer. Reset some
	 * ip fields to reflect the state of this buffer.
	 * 
	 * @param ip
	...
	/**
	 * Adds a IP fragment to the buffer. This fragment is also the last framgent
	 * of the fragment series which carries special information about the
	 * original IP datagram.
	 * 
	 * @param packet
	 *            a packet buffer containing IP fragment data
	 * @param offset
	 *            offset into this buffer where the fragment data should be
	 *            copied to
	 * @param length
	 *            the length of the fragment data
	 * @param packetOffset
	 *            offset into the packet buffer where fragment data begins
	 */
	...
	/**
	 * Adds a IP fragment to the buffer. The fragment data is copied into this
	 * buffer at specified offset from the supplied packet data buffer.
	 * 
	 * @param packet
	 *            a packet buffer containing IP fragment data
	 * @param offset
	...
	public void addSegment(JBuffer packet, int offset, int length,
		...
		/*
		 * Keep track of how much data we're copied so far. Needed to determine
		 * if the reassembly process is complete.
		 */
	 ...
	 * specifically useful when using a PriorityQueue which will order the
	 * buffers for us according to the timeout timestamp. The oldest buffers are
	 * on top of the queue, while the youngest are at the bottom.
	 * 
	 * @param o
	 *            the other buffer
	 * @return the difference between the two buffers
	 */
	...
	/**
	 * Checks if the buffer reassembly is complete. If the number of bytes
	 * copied into this buffer including the ip header up front, equals the
	 * length of the original IP datagram, that means the fragmentation
	 * succeeded and is complete.
	 * 
	...
	/**
	 * Compares the timeout timestamp against the current time. If timeout
	 * timestamp is still in the future, then it returns false. If the timestamp
	 * is in the past, then true is returned and buffer is considered timedout.
	 * 
	...
	/**
	 * Returns the working Ip4 header instance found at the front of this buffer
	 * 
